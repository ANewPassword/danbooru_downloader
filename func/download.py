...
from func.fileio import file_mkdir, file_is_exist, file_write_stream, file_write, file_delete, file_size
                        ...
                        while True:
                            try:
                                if continue_point:
                                    break
                                http = Session()
                                res =  http.get(post_url, proxies = self.proxy, headers = post_header, stream = True, timeout = 3600, impersonate = "chrome136")
                                task_length = int(res.headers.get('content-length', 0))
                                if res.status_code == 200:
                                    if not file_write_stream('{}{}{}'.format(self.path, '/', post_file_name), res, 3600):
                                        if err_count < self.retry_max or self.retry_max == -1:
                                            err_count += 1
                                            add_log('%s 写入失败或超时, 正在第 %s 次重试' % (post_file_name, err_count), 'Warn')
                                            sleep(1)
                                            continue
                                        else:
                                            add_log('%s 写入失败或超时, 超过最大重试次数' % (post_file_name), 'Error')
                                    ...
                                    sleep(1 if res.status_code != 429 else 10 * err_count)
                                    continue
                                else:
                                    continue_point = True
...
def start_download(download_info, thread, path, proxy_address, retry_max, deduplication, chksums, with_metadata, program_path, template_name): # 下载器接口
    ...
    for i in range(thread):
        ...
        d.start()
    while True: # hold
        # print('download_count:', download_count) # debug
